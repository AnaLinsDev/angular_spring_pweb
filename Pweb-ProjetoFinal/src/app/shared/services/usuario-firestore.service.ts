...
export class UsuarioFirestoreService {
 ...
 pesquisarPorId(id: string): Observable<Usuario> {
   ...
   //  para o tipo usuário
   return this.colecaoUsuarios.doc(id).get().pipe(map(document => new Usuario(Number(document.id), document.data())));
 }
 ...
 atualizar(usuario: Usuario): Observable<void> {
   ...
   delete usuario.id;
   return from(this.colecaoUsuarios.doc(usuario.id.toString()).update(Object.assign({}, usuario)));
 }
